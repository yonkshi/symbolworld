...
class LargerObject(things.LargeSprite):
  ...
  def __init__(self, img, corner, position, character, impassable,
    ...
    Args:
      ...
      shape: Shape of the object, a dictionary of colors. #FIXME Might use a different format later
  ...
  def _pos_rel2abs(self, rel_pos):
    """
    Method for converting relative position to absolute position in image
    :return: absolute position of pixel in image
    """
    return self._virtual_row + rel_pos[0], self._virtual_col + rel_pos[1]
  ...
  def _check_motion(self, board, motion):
    ...
    def new_loc_char(loc):
      ''' Returns the surrounding character of all pixels '''
      surrounding_chars = []
      for coords, rgb in self.img.items():
        new_coords = (loc[0] + coords[0], loc[1] + coords[1])
        surrounding_chars.append(at(new_coords))
    ...
    if motion == self._STAY:
      ...
      neighbs = at(self._WEST), at(self._NORTHWEST), at(self._NORTH), new_loc_char(motion)
    elif motion == self._NORTH:        # ↑
      neighbs = at(self._NORTH), new_loc_char(motion)
    elif motion == self._NORTHEAST:    # ↗
      neighbs = at(self._NORTH), at(self._NORTHEAST), at(self._EAST), new_loc_char(motion)
    elif motion == (self._EAST):       # →
      neighbs = at(self._EAST), new_loc_char(motion)
    elif motion == (self._SOUTHEAST):  # ↘
      neighbs = at(self._EAST), at(self._SOUTHEAST), at(self._SOUTH), new_loc_char(motion)
    elif motion == (self._SOUTH):      # ↓
      neighbs = at(self._SOUTH), new_loc_char(motion)
    elif motion == (self._SOUTHWEST):  # ↙
      neighbs = at(self._SOUTH), at(self._SOUTHWEST), at(self._WEST), new_loc_char(motion)
    elif motion == (self._WEST):       # ←
      neighbs = at(self._WEST), new_loc_char(motion)
    else:
    ...
    if all(motion):  # If the motion is diagonal:
      ...
      for n_char in neighbs[3]: # Check all pixels of object
        if is_impassable(n_char): return neighbs
    else:  # Otherwise, if the motion is "cardinal":
      if is_impassable(neighbs[0]): return neighbs
      for n_char in neighbs[1]:  # Check all pixels of object
        if is_impassable(n_char): return neighbs
